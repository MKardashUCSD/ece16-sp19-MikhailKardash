...
if write_flag:
    write_filename = "data_train_E.csv"
...
def get_data():  
    ...
    try:
        ...
        data = [float(t.strip()), float(y.strip()),float(imu.strip())]     # Strip spaces
    except:
        pass
...
def update_data():
    ...
    data_buffer[4].add(data[2]) #to be filtered later
    ...
    return [(data_buffer[0], data_buffer[1]),(data_buffer[0], data_buffer[2]),(data_buffer[0], data_buffer[3])]
...
while True:
    try:
        ...
        #filter3 = Filter(sampling_frequency=sampling_freq, filter_frequency=.75, filter_type='high')
