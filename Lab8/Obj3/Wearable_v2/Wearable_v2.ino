...
void loop() {
  ...
  if (confirmedCentral) {                              // This implicitly checks if BLE is asleep (confirmedCentral would be false if asleep==true)
    ...
    if (endTime - startTime >= period_us) {
      if(imuDataReady) {
        readIMU();
        imuVal = abs(ax) + abs(ay) + abs(az);
      }
      tim = millis()-setupTime;
      startTime = endTime;
      reading = analogRead(A2);
      if (tim%1000 > 100) {
        sprintf(bufferBOI,"%d.%d,%d,%d;",(int)(tim/1000),(int)(tim%1000),reading,imuVal);
      }
      else {
        sprintf(bufferBOI,"%d.0%d,%d,%d;",(int)(tim/1000),(int)(tim%1000),reading,imuVal);
      }
      BTserial.write(bufferBOI);     // Combine char arrays and add coma, semicolon
    }
  }
}

void checkVibe(const char * message){
 if (message[0] == '^'){
